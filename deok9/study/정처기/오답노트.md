...
- 4. 자료 사전(DD)의 작성 목적은 무엇인지
	- 조직에 속해 있는 다른 사람들에게 특정한 자료 용어가 무엇을 의미하는지를 알려주기 위하여, 용어의 정의를 조정하고 취합하고 문서로 명확히 하는 목적이 있다.
- 5. DB의 트랜잭션(Transaction)에 대해 서술
	- 트랜잭션은 DB 시스템에서 하나의 논리적 기능을 정상적으로 수행하기 위한 작업의 기본 단위이다
- 6. revoke insert on DEPT from 신민아;
	- 사용자 신만아에게 DEPT 테이블 insert 권한 회수
- 7. drop view 학생;
	- 뷰(view)는 객체이므로 DDL을 이용하여 삭제한다. (DROP).
- 8-1. 스프린트(Sprint)
	- 2~4주의 짧은 개발 기간
- 8-2. 번 다운 차트(Burn Down Chart)
